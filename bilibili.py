...
class bilibili():
    ...
    def down_video(self,video_list, title, start_url, page):
        ...
        for i in video_list:
            ...
            # 开始下载
            if len(video_list) > 1:
                urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}-{}.flv'.format(title, num)),reporthook=self.Schedule_cmd)  # 写成mp4也行  title + '-' + num + '.flv'
            else:
                urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}.flv'.format(title)),reporthook=self.Schedule_cmd)  # 写成mp4也行  title + '-' + num + '.flv'
            num += 1
